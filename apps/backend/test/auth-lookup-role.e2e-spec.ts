...
describe("village_app_auth_lookup role — blast-radius verification", () => {
  ...
  beforeAll(async () => {
    ...
    authLookupPrisma = new PrismaClient({
      datasourceUrl: process.env.AUTH_LOOKUP_DATABASE_URL,
    });
  }, 30_000);
  ...
  it("CANNOT read password_hash", async () => {
    await expect(
      authLookupPrisma.user.findMany({
  ...
  it("CANNOT do an implicit SELECT * (Prisma's default findMany() with no select)", async () => {
    await expect(authLookupPrisma.user.findMany()).rejects.toThrow(
      /permission denied/i,
    );
  });

  it("CANNOT read visitor_passes (no grant on this or any other table)", async () => {
    await expect(authLookupPrisma.visitorPass.findMany()).rejects.toThrow(
      /permission denied/i,
    );
  });

  it("CANNOT read audit_logs", async () => {
    await expect(authLookupPrisma.auditLog.findMany()).rejects.toThrow(
      /permission denied/i,
    );
  });

  it("CANNOT read houses", async () => {
    await expect(authLookupPrisma.house.findMany()).rejects.toThrow(
      /permission denied/i,
    );
  });

  it("CANNOT read entry_logs", async () => {
    await expect(authLookupPrisma.entryLog.findMany()).rejects.toThrow(
      /permission denied/i,
    );
  });
  ...
  it("CANNOT write — UPDATE is rejected even on the users table it can read from", async () => {
    await expect(
      authLookupPrisma.user.updateMany({
        where: { id: villageA.resident.id },
        data: { name: "hacked" },
      }),
    ).rejects.toThrow(/permission denied/i);
  });
