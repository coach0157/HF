...
describe('Security & acceptance-criteria flows (API-driven e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    (ThrottlerGuard.prototype as unknown as CanActivate).canActivate = async () => true;
  ...
  function passBody(visitorName: string, usageType: 'SINGLE' | 'MULTI' = 'SINGLE') {
    return {
      visitorName,
  ...
  describe('Cross-tenant isolation via API', () => {
    it('village A cannot revoke a visitor pass created in village B (404, not 403 — RLS hides the row entirely)', async () => {
      ...
      const revokeRes = await api(baseUrl, 'PATCH', `/visitor-passes/${passRes.body.id}/revoke`, {
    ...
    it('village A admin cannot view a user record from village B', async () => {
      ...
      const res = await api(baseUrl, 'GET', `/users/${villageB.resident.id}`, { token: adminAToken });
      expect(res.status).toBe(404);
    });

    it('village B cannot see or read village A entry logs', async () => {
      const residentAToken = await loginToken(baseUrl, villageA.resident.phone, villageA.villageId);
      const guardAToken = await loginToken(baseUrl, villageA.guardOnDuty.phone, villageA.villageId);
      ...
      const listRes = await api(baseUrl, 'GET', '/entry-logs', { token: adminBToken });
      expect(listRes.status).toBe(200);
      expect(listRes.body.items.some((i: any) => i.id === entryLogId)).toBe(false);

      const getOneRes = await api(baseUrl, 'GET', `/entry-logs/${entryLogId}`, { token: adminBToken });
    ...
    it('village B does not see village A SOS alerts', async () => {
      ...
      const adminBToken = await loginToken(baseUrl, villageB.admin.phone, villageB.villageId);
      const listRes = await api(baseUrl, 'GET', '/sos-alerts', { token: adminBToken });
      expect(listRes.status).toBe(200);
      expect(listRes.body.some((a: any) => a.id === sosRes.body.alert.id)).toBe(false);
    });
  });
  ...
  describe('RBAC (403 for wrong role)', () => {
    ...
    it('resident cannot acknowledge an SOS alert (guard only)', async () => {
      const residentAToken = await loginToken(baseUrl, villageA.resident.phone, villageA.villageId);
      const sosRes = await api(baseUrl, 'POST', '/sos-alerts', { token: residentAToken, body: {} });
      const res = await api(baseUrl, 'PATCH', `/sos-alerts/${sosRes.body.alert.id}/acknowledge`, {
        token: residentAToken,
      });
      expect(res.status).toBe(403);
    });
    ...
    it('guard cannot create a visitor pass (resident only)', async () => {
      const guardAToken = await loginToken(baseUrl, villageA.guardOnDuty.phone, villageA.villageId);
      const res = await api(baseUrl, 'POST', '/visitor-passes', { token: guardAToken, body: passBody('x') });
      expect(res.status).toBe(403);
    });

    it('resident cannot list guard shifts (admin only)', async () => {
      const residentAToken = await loginToken(baseUrl, villageA.resident.phone, villageA.villageId);
      const res = await api(baseUrl, 'GET', '/guard-shifts', { token: residentAToken });
      expect(res.status).toBe(403);
    });
  ...
  describe('Exit-confirm flow (spec 2.1: no auto-close on re-scan)', () => {
    it('re-scanning the QR at the exit gate does NOT set exit_time; only an explicit confirm-exit call does', async () => {
      ...
      const readRes = await api(baseUrl, 'GET', `/entry-logs/${entryLogId}`, { token: guardAToken });
      expect(readRes.body.exitTime).toBeNull();

      // The explicit confirm step.
      const confirmRes = await api(baseUrl, 'PATCH', `/entry-logs/${entryLogId}/confirm-exit`, {
        token: guardAToken,
      });
      ...
      const confirmAgainRes = await api(baseUrl, 'PATCH', `/entry-logs/${entryLogId}/confirm-exit`, {
        token: guardAToken,
      });
    ...
    it('a resident of a different village cannot confirm exit for someone else\'s visitor (cross-tenant, 404)', async () => {
      ...
      const residentBToken = await loginToken(baseUrl, villageB.resident.phone, villageB.villageId);
      const res = await api(baseUrl, 'PATCH', `/entry-logs/${entryLogId}/confirm-exit`, { token: residentBToken });
      expect(res.status).toBe(404);
    });
  });
  ...
  describe('QR revoke', () => {
    it('a revoked pass cannot be scanned or used for entry, even before its natural expiry', async () => {
      ...
      const revokeRes = await api(baseUrl, 'PATCH', `/visitor-passes/${passId}/revoke`, { token: residentAToken });
      expect(revokeRes.status).toBe(200);
      expect(revokeRes.body.status).toBe('REVOKED');

      const scanRes = await api(baseUrl, 'GET', `/visitor-passes/${qrToken}`, { token: guardAToken });
      expect(scanRes.status).toBe(403);

      const entryRes = await api(baseUrl, 'POST', '/entry-logs', { token: guardAToken, body: { qrToken } });
    ...
    it('revoke is idempotent, and revoking twice does not error', async () => {
      ...
      const first = await api(baseUrl, 'PATCH', `/visitor-passes/${passRes.body.id}/revoke`, {
        token: residentAToken,
      });
      const second = await api(baseUrl, 'PATCH', `/visitor-passes/${passRes.body.id}/revoke`, {
        token: residentAToken,
      });
  ...
  describe('SOS routing (spec 2.2: on_duty guards only)', () => {
    it('routes only to the on-duty guard, excludes the off-duty guard', async () => {
      const onDutyToken = await loginToken(baseUrl, villageA.guardOnDuty.phone, villageA.villageId);
      const offDutyToken = await loginToken(baseUrl, villageA.guardOffDuty.phone, villageA.villageId);

      const startOnDuty = await api(baseUrl, 'POST', '/guard-shifts', { token: onDutyToken, body: {} });
      ...
      const startOffDuty = await api(baseUrl, 'POST', '/guard-shifts', { token: offDutyToken, body: {} });
      const endOffDuty = await api(baseUrl, 'PATCH', `/guard-shifts/${startOffDuty.body.id}`, {
        token: offDutyToken,
      });
      ...
      expect(sosRes.status).toBe(201);
      expect(sosRes.body.routedToGuardUserIds).toContain(villageA.guardOnDuty.id);
      expect(sosRes.body.routedToGuardUserIds).not.toContain(villageA.guardOffDuty.id);

      const ackRes = await api(baseUrl, 'PATCH', `/sos-alerts/${sosRes.body.alert.id}/acknowledge`, {
        token: onDutyToken,
      });
      ...
      await api(baseUrl, 'PATCH', `/guard-shifts/${startOnDuty.body.id}`, { token: onDutyToken });
    });
  });
    ...
    it('admin revoking another user\'s pass writes an audit_logs row; the owner revoking their own does not', async () => {
      ...
      const logs = await withVillageContext(villageA.villageId, (tx) =>
        tx.auditLog.findMany({
          where: { resourceId: passRes.body.id, action: 'REVOKE_VISITOR_PASS_OTHER_USER' },
        }),
      );
      ...
      const selfRevokeRes = await api(baseUrl, 'PATCH', `/visitor-passes/${passRes2.body.id}/revoke`, {
        token: residentAToken,
      });
  ...
  describe('Multi-village phone number (documented Dev gap: returns 409, does not silently pick a village)', () => {
    it('login without villageId returns 409 with the candidate villages when the phone exists in more than one', async () => {
      ...
      await withVillageContext(villageA.villageId, (tx) =>
        tx.user.create({ data: { villageId: villageA.villageId, name: 'Shared A', phone: sharedPhone, role: 'RESIDENT' } }),
      );
      await withVillageContext(villageB.villageId, (tx) =>
        tx.user.create({ data: { villageId: villageB.villageId, name: 'Shared B', phone: sharedPhone, role: 'RESIDENT' } }),
      );
      ...
      const disambiguated = await loginAs(baseUrl, sharedPhone, villageA.villageId);
