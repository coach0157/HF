 ...
 * (apps/backend/src/common/files/). Every page that renders a photo from
 * one of those fields must go through this helper instead of passing the
 * raw ref straight to `<img src>` — a browser can't load the `local://`
 * scheme at all.
 *
 * The access token is appended as `?token=`, not an `Authorization` header,
 * because a plain `<img>` tag fires its own request and can't attach a
 * custom header — see the backend endpoint's own doc comment / ADR-007 for
 * why that's safe to accept on this one route.
 */
